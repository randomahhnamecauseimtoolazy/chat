...
local baseUrl = "https://raw.githubusercontent.com/randomahhnamecauseimtoolazy/chat/refs/heads/main/"

-- // Load Modules \\
local Settings = loadstring(game:HttpGet(baseUrl .. "settings.lua"))()
local Utilities = loadstring(game:HttpGet(baseUrl .. "utilities.lua"))()
local Aimbot = hasMouseMoveRel and loadstring(game:HttpGet(baseUrl .. "aimbot.lua"))() or nil
...
if Aimbot then Aimbot:Init(Settings, State, Utilities) end
...
UI:Init(Settings, State, Library, hasMouseMoveRel, Aimbot, ESP, FOV, Chams, Crosshair, PlayerMod, Misc, Utilities)

-- // Cleanup \\
Library:OnUnload(function()
    if Aimbot then Aimbot:Cleanup() end
    ESP:Cleanup()
    FOV:Cleanup()
    Chams:Cleanup()
    Crosshair:Cleanup()
